...
- (void)setPublicDomain:(NSInteger)action;
{
    ...
    [self setObject:requestId forKey:@"id"]; //添加ID
...
- (NSString *)getSign:(NSString *)timestamp requestId:(NSString *)requestId
{
     ...
     */
    NSLog(@"2222: %@",[NSString stringWithFormat:@"%@%@%@",[self getRequestId:timestamp],timestamp,kTerminal_no]);
...
NSString *md5(NSString* source){
    /* 服务器选择 */
    NSString *md5Salt = MD5_MARCO;
    NSLog(@"-------%@------------------",md5Salt);
    source = [NSString stringWithFormat:@"%@%@",source,md5Salt];
